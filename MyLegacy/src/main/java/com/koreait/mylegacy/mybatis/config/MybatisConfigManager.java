/*
 * 이 클래스의 목적 : 
 * 쿼리문 수행에 필요한SqlSession을 보다 쉽게 얻어갈 수 있도록 재사용성을 고려하여
 * 정의한 객체, 특히 이 객체의 인스턴스는 어플리케이션내에서 1개만 둬야 하므로 Singleton
 * 패턴으로 정의한다!!
 * */
...
public class MybatisConfigManager {
	...
	//3)getter는 제공했으나, 역시 인스턴스 메서드이므로 new한 다음에나 호출할 수 있기 때문에 
	//그 어떤 객체로 이 메서드를 호출할 수 없다.(new를 막아놓았으니)
	//해결책? new 하지 않고도 아래의 메서드를 호출할 수 있도록 static 메서드로 정의하자!
	public static MybatisConfigManager getInstance() {
		//4) 이 메서드 호출시 instance 변수가 null이라면, 여기서 인스턴스를 생성해서
		//null 값이 아닌걸 가져가도록 하자!! 
		if(instance == null) {
